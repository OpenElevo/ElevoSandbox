...
use crate::proto::{
    ...
    agent_message, server_message, AgentMessage, ServerMessage,
    ServerHandshakeAck, ServerHeartbeatAck,
};
                ...
                Some(Ok(msg)) => {
                    match msg.message {
                        Some(agent_message::Message::Handshake(handshake)) => {
                            info!("Agent handshake received for sandbox: {}", handshake.sandbox_id);
                            ...
                            let _ = tx_clone.send(Ok(ServerMessage {
                                message: Some(server_message::Message::HandshakeAck(ServerHandshakeAck {
                                    success: true,
                                    error: None,
                                })),
                            })).await;
                    ...
                    Ok(msg) => {
                        match msg.message {
                            ...
                            Some(agent_message::Message::CommandResponse(resp)) => {
                                debug!("Command response from {}: {}", sandbox_id, resp.correlation_id);
                                agent_pool.handle_response(&sandbox_id, resp);
                            }
                            Some(agent_message::Message::PtyOutput(output)) => {
                                debug!("PTY output from {}: {} bytes", sandbox_id, output.data.len());
...
fn convert_pool_message_to_proto(msg: AgentMessageType) -> ServerMessage {
    use crate::proto::{
        AgentRunCommandRequest, AgentKillProcessRequest, AgentCreatePtyRequest,
        AgentResizePtyRequest, AgentKillPtyRequest, AgentPtyInput,
    };

    let message = match msg {
        AgentMessageType::RunCommand { correlation_id, command, args, env, cwd, timeout_ms, stream } => {
        ...
        AgentMessageType::KillProcess { correlation_id, pid, signal } => {
        ...
        AgentMessageType::CreatePty { correlation_id, pty_id, cols, rows, shell, env } => {
        ...
        AgentMessageType::ResizePty { correlation_id, pty_id, cols, rows } => {
        ...
        AgentMessageType::KillPty { correlation_id, pty_id } => {
        ...
        AgentMessageType::PtyInput { pty_id, data } => {
            server_message::Message::PtyInput(AgentPtyInput {
                pty_id,
                data,
            })
        }
        AgentMessageType::HandshakeAck { success, error } => {
            server_message::Message::HandshakeAck(ServerHandshakeAck {
                success,
                error,
            })
        }
        AgentMessageType::HeartbeatAck { timestamp } => {
            server_message::Message::HeartbeatAck(ServerHeartbeatAck {
                timestamp,
            })
        }
    };

    ServerMessage { message: Some(message) }
}
